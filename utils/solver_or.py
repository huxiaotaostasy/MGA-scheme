...
def make_model_optimizer(config, model, num_gpu=None):
    if num_gpu is None:
        base_lr = config.SOLVER.BASE_LR
        refine_lr = config.SOLVER.REFINE_LR
    else:
...
def make_err_optimizer(config, model, num_gpu=None):
    if num_gpu is None:
        base_lr = config.SOLVER.BASE_LR
        err_lr = config.SOLVER.ERR_LR
    else:
        base_lr = config.SOLVER.BASE_LR * num_gpu
        err_lr = config.SOLVER.ERR_LR * num_gpu
    if config.SOLVER.OPTIMIZER == 'Adam':
        optimizer = optim.Adam([
                                {'params': model.backbone.parameters(), 'lr': base_lr},
                                {'params': model.conv_err.parameters(), 'lr': err_lr},
                                ],
                               betas=(0.9, 0.999), eps=1e-8,
                               weight_decay=config.SOLVER.WEIGHT_DECAY)
    elif config.SOLVER.OPTIMIZER == 'SGD':
...
def make_lr_scheduler(config, optimizer):
    ...
    return scheduler
class CosineAnnealingLR_warmup_err(_LRScheduler):
    def __init__(self, config, optimizer, fisrt_lr, second_lr, last_epoch=-1, min_lr=1e-7):
        ...
        super(CosineAnnealingLR_warmup_err, self).__init__(optimizer, last_epoch=last_epoch)
    ...
    def get_lr(self):
        if self.last_epoch - self.last_restart < self.w_iter:
            ...
            return [0, (self.second_lr - self.min_lr) * ratio + self.min_lr]
...
class CosineAnnealingLR_warmup_two(_LRScheduler):
    def __init__(self, config, optimizer, fisrt_lr, second_lr, last_epoch=-1, min_lr=1e-7):
        self.fisrt_lr = fisrt_lr
        self.second_lr = second_lr
        self.min_lr = min_lr
        ...
        self.T_max = self.T_period[0]
        assert config.SOLVER.MAX_ITER == self.T_period[-1], 'Illegal training period setting.'
        super(CosineAnnealingLR_warmup_two, self).__init__(optimizer, last_epoch=last_epoch)
